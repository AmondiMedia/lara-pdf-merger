...
class TCPDF {
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		//while (list($key, $val) = each($prop)) { - AMONDI PATCH PHP7.2
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					// while (list($id, $name) = each($attr_array[1])) { - AMONDI PATCH PHP7.2
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// while (list($id, $name) = each($style_array[1])) { - AMONDI PATCH PHP7.2
